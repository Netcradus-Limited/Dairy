...
void main() {
  group('Admin Subscription Management Tests', () {
    ...
    test('1. Stream all subscriptions from root subscriptions collection with multiple subscriptions per customer', () async {
      final now = DateTime.now();

      // Seed 3 subscriptions for the SAME customer in root subscriptions collection
      ...
      await fakeFirestore.collection('subscriptions').doc(subMilk.id).set(subMilk.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(subGhee.id).set(subGhee.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(subPaneer.id).set(subPaneer.toFirestore());

      // Read via streamAllSubscriptions
      final subs = await subscriptionService.streamAllSubscriptions().first;

      expect(subs.length, 3);
      expect(subs.map((s) => s.id).toSet(), {'sub_milk_001', 'sub_ghee_002', 'sub_paneer_003'});
    ...
    test('2. Admin metrics and KPI calculations', () async {
      ...
      await fakeFirestore.collection('subscriptions').doc(sub1.id).set(sub1.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(sub2.id).set(sub2.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(sub3.id).set(sub3.toFirestore());
    ...
    test('3. Admin lifecycle actions: pause, resume, cancel, and edit update exact subscription doc', () async {
      final now = DateTime.now();
      ...
      await fakeFirestore.collection('subscriptions').doc(sub.id).set(sub.toFirestore());

      // 1. Admin Pause
      await adminProvider.pauseSubscription('sub_action_001');
      var doc = await fakeFirestore.collection('subscriptions').doc('sub_action_001').get();
      expect(doc.data()!['status'].toString().toLowerCase(), 'paused');

      // 2. Admin Resume
      await adminProvider.resumeSubscription('sub_action_001');
      doc = await fakeFirestore.collection('subscriptions').doc('sub_action_001').get();
      ...
      expect(doc.data()!['frequency'].toString().toLowerCase(), contains('alternate'));
    ...
    test('4. Order integration & delivery history for subscription', () async {
      ...
      final history = await subscriptionService.streamOrdersForSubscription('sub_milk_001').first;
    ...
    test('6. Status and frequency filtering on subscriptions', () async {
      ...
      final activeOnly = list.where((s) => s.status == SubscriptionStatus.active).toList();
      final dailyOnly = list.where((s) => s.frequency == SubscriptionFrequency.daily).toList();
    ...
    test('7. Multiple subscriptions with same userId have unique subscription document IDs', () async {
      final now = DateTime.now();
      ...
      await fakeFirestore.collection('subscriptions').doc(s1.id).set(s1.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(s2.id).set(s2.toFirestore());
      await fakeFirestore.collection('subscriptions').doc(s3.id).set(s3.toFirestore());
